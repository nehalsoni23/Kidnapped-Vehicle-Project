	...
	for(int p = 0; p < num_particles; p++) {
		...
		for(int i = 0; i < trans_observations.size(); i++) {
			...
			for(int j = 0; j < map_landmarks.landmark_list.size(); j++){
				...
				double calculated_dist = sqrt(pow(trans_observations[i].x - x_l, 2.0) + pow(trans_observations[i].y - y_l, 2.0));
			...
			if (association != 0) {
				...
				// calculating Multivariate-Gaussian probability
				...
				// Multiplying calculated measurement probabilities to get the Particle's final weight
				if (multiplier > 0) {
